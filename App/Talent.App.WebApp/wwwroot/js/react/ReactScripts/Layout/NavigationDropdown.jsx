...
export function NavigationDropdown(props) {
  const classes = props.right
    ? "ui dropdown link right item"
    : "ui dropdown link item";
  const tags = props.items.map((x) => (
    <NavigationLink key={x.label} href={x.href} label={x.label} />
  ));
  return (
...
export function NavigationButton(props) {
  ...
  return (
    <div className={classes}>
      <a className="ui teal button" href={props.href} onClick={props.onClick}>
        {props.label}
      </a>
    </div>
  );
