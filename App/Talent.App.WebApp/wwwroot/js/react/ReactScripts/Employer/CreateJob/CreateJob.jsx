...
export default class CreateJob extends React.Component {
  ...
  loadData() {
    ...
    var copyJobParam = this.props.match.params.copyId
    ...
    if (param != "" || copyJobParam != "") {
      var link =
        param != ""
          ? "https://ctalentservices.azurewebsites.net/listing/listing/GetJobByToEdit?id=" +
            param
          : "https://ctalentservices.azurewebsites.net/listing/listing/GetJobForCopy?id=" +
            copyJobParam;
      ...
      $.ajax({
        ...
        success: function (res) {
          if (res.success == true) {
            res.jobData.jobDetails.startDate = moment(
              res.jobData.jobDetails.startDate
            );
            res.jobData.jobDetails.endDate = res.jobData.jobDetails.endDate
              ? moment(res.jobData.jobDetails.endDate)
              : null;
            res.jobData.expiryDate = res.jobData.expiryDate
              ? moment(res.jobData.expiryDate) > moment()
                ? moment(res.jobData.expiryDate)
                : moment().add(14, "days")
              : null;
  ...
  addUpdateJob() {
    ...
    $.ajax({
      url:
        "https://ctalentservices.azurewebsites.net/listing/listing/createUpdateJob",
      headers: {
        Authorization: "Bearer " + cookies,
        "Content-Type": "application/json",
      },
                    ...
                    <div className="ui grid">
                      <div className="row">
                        <div className="twelve wide column">
                          <label>
                            * are required fields. Please enter all required
                            fields.
                          </label>
                          <h5>*Title:</h5>
                          <ChildSingleInput
                          ...
                          <JobApplicant
                            applicantDetails={
                              this.state.jobData.applicantDetails
                            }
                            updateStateData={this.updateStateData}
                          />
                          <br />
